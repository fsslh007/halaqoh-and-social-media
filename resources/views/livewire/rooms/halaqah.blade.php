<div>
    ...
    <div>
        <div class="mt-8 flex justify-center">
            <h1 class="w-full max-w-4xl py-4 bg-white shadow-md rounded-lg text-center font-bold text-2xl sm:text-3xl lg:text-4xl px-4 sm:px-8 md:px-12">
                MY HALAQAH
            </h1>
        </div>
        <div class="w-full px-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
                @foreach ($rooms->load('members') as $room)
                    @if ($room->members && $room->members->contains('user_id', $currentUserId))
                        <div class="w-full sm:w-1/2 md:w-1/3 p-4">
                            <div class="bg-white shadow-md overflow-hidden rounded-lg h-full flex flex-col items-center justify-center">
                                ...
                                <!-- Join Halaqah Button -->
                                <div class="mt-4">
                                    <x-jet-button wire:click="seeHalaqah('{{ $room->id }}', '{{ $room->name }}')">
                                        {{ __('See Halaqah') }}
                                    </x-jet-button>
                                </div>
